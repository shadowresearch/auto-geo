...
export const seedResource: ResourcePublishPayload = {
  slug: "hello-auto-geo",
  title: "Hello from the auto-geo Fastify example",
  metaDescription:
    "A seeded resource that demonstrates the auto-geo publish flow running on a Fastify server, served entirely from the in-memory storage adapter.",
  category: "Examples",
  excerpt:
  ...
  keywords: ["auto-geo", "fastify", "node"],
  geoMetadata: {
    targetQueries: [
      "auto-geo fastify example",
      "how to run auto-geo on fastify",
      "auto-geo endpoint only example",
    ],
    pageType: "resource",
    primaryFunction:
      "Demonstrate the auto-geo publish endpoint wired into a Fastify server using runPublish directly, with no framework-specific adapter.",
    optimizationFramework: ["GEO"],
    targetPlatforms: ["chatgpt", "perplexity"],
    informationGainStatement:
      "First-party reference for wiring auto-geo into Fastify by calling runPublish from the package directly, with the in-memory store for local development.",
    refreshCadence: "quarterly",
  },
  about: [{ name: "auto-geo" }, { name: "Fastify" }, { name: "Node.js" }],
  tldr: {
    text: "This seeded resource exists so the auto-geo Fastify example has a non-empty store on first run, which lets the GET listing endpoint and GET by slug endpoint return useful JSON before you have published any payload of your own to the running server.",
  },
  intro: {
    blocks: [
      {
        type: "paragraph",
        text: "The auto-geo Fastify example calls runPublish from the auto-geo package directly to handle POST and DELETE, plus a small in-memory store seeded with this payload so reads work immediately. Replace it by posting your own payload to /api/resources/publish.",
      },
    ],
  },
  sections: [
  ...
  keyTakeaways: {
    items: [
      "Fastify route handlers can call runPublish from the auto-geo package directly without a custom adapter.",
  ...
  faq: {
    items: [
      {
        question: "Why does this example not use a Fastify-specific adapter?",
        answer:
          "auto-geo ships dedicated adapters for Next.js and Hono today, with more on the roadmap. For Fastify, the cleanest integration is to call runPublish from the package directly inside a route handler. That is the pattern this example documents end to end against the in-memory store.",
      },
      {
        question: "Can I add Fastify plugins for auth or rate limiting?",
        answer:
          "Yes. The example uses the simplest auth shape for clarity, but you can register fastify-rate-limit, fastify-jwt, or your own auth plugin and have the publish route run inside the protected scope. runPublish itself does not care about how authentication is performed.",
      },
      {
  ...
  disclosure: {
    text: "This is a seed payload included with the auto-geo Fastify example. It satisfies the hard schema constraints but is deliberately short for demonstration purposes and will surface soft warnings on republish, which mirrors the real iteration loop.",
  },
};
