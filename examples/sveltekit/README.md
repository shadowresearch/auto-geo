# sveltekit — auto-geo in a SvelteKit endpoint

Endpoint-only example. SvelteKit `+server.ts` handlers call `runPublish`
and `runDelete` from the package directly — there is no SvelteKit-
specific auto-geo adapter today.

> This example does NOT render auto-geo content as HTML (auto-geo ships
> a React renderer only). For the full React render path see
...
curl http://localhost:3003/api/resources/hello-auto-geo
```
...
```bash
curl -X POST http://localhost:3003/api/resources/publish \
...
```bash
curl http://localhost:3003/api/resources
curl http://localhost:3003/api/resources/publish-test
```
...
| File                                          | What it does                                                           |
| --------------------------------------------- | ---------------------------------------------------------------------- |
| `src/routes/api/resources/publish/+server.ts` | POST + DELETE handlers calling `runPublish` / `runDelete` directly     |
| `src/routes/api/resources/+server.ts`         | GET handler that lists every stored resource                           |
| `src/routes/api/resources/[slug]/+server.ts`  | GET handler that fetches a single stored resource as raw JSON          |
| `src/lib/auto-geo.ts`                         | Shared store + site config — instantiated once and imported everywhere |
| `src/lib/seed.ts`                             | Inline valid seed payload so the store is non-empty on first run       |

## Notes

- Uses `@sveltejs/adapter-node` so it runs anywhere Node runs. Swap the
  adapter if you deploy to Cloudflare, Vercel, etc.
