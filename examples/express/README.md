# express — auto-geo on Express

Endpoint-only example. There is no official Express adapter for
`auto-geo` today, so this example calls `runPublish` and `runDelete`
from the package directly. The pattern is the canonical reference for
any Node HTTP framework without a dedicated adapter.
...
```bash
curl http://localhost:3002/api/resources
curl http://localhost:3002/api/resources/hello-auto-geo
```
...
```bash
curl -X POST http://localhost:3002/api/resources/publish \
...
```bash
curl http://localhost:3002/api/resources
curl http://localhost:3002/api/resources/publish-test
```
...
| File        | What it does                                                                             |
| ----------- | ---------------------------------------------------------------------------------------- |
| `server.ts` | Express app: calls `runPublish` / `runDelete` directly, adds GET endpoints for verifying |
| `seed.ts`   | Inline valid seed payload so the store is non-empty on first run                         |

## Notes

- This same `runPublish` + `runDelete` pattern works for any Node HTTP
  framework (Koa, h3, raw `http.createServer`, etc.). Translate the
  request/response idioms; the call into `auto-geo` stays the same.
