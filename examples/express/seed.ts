...
export const seedResource: ResourcePublishPayload = {
  slug: "hello-auto-geo",
  title: "Hello from the auto-geo Express example",
  metaDescription:
    "A seeded resource that demonstrates the auto-geo publish flow running on an Express server, served entirely from the in-memory storage adapter.",
  category: "Examples",
  excerpt:
  ...
  keywords: ["auto-geo", "express", "node"],
  geoMetadata: {
    targetQueries: [
      "auto-geo express example",
      "how to run auto-geo on express",
      "auto-geo endpoint only example",
    ],
    pageType: "resource",
    primaryFunction:
      "Demonstrate the auto-geo publish endpoint wired into a vanilla Express server using runPublish directly, with no framework-specific adapter.",
    optimizationFramework: ["GEO"],
    targetPlatforms: ["chatgpt", "perplexity"],
    informationGainStatement:
      "First-party reference for wiring auto-geo into Express by calling runPublish from the package directly, with the in-memory store for local development.",
    refreshCadence: "quarterly",
  },
  about: [{ name: "auto-geo" }, { name: "Express" }, { name: "Node.js" }],
  tldr: {
    text: "This seeded resource exists so the auto-geo Express example has a non-empty store on first run, which lets the GET listing endpoint and GET by slug endpoint return useful JSON before you have published any payload of your own to the running server.",
  },
  intro: {
    blocks: [
      {
        type: "paragraph",
        text: "The auto-geo Express example calls runPublish from the auto-geo package directly to handle POST and DELETE, plus a small in-memory store seeded with this payload so reads work immediately. Replace it by posting your own payload to /api/resources/publish.",
      },
    ],
  },
  sections: [
  ...
  keyTakeaways: {
    items: [
      "auto-geo ships runPublish and runDelete as framework-agnostic functions for backends without a dedicated adapter.",
  ...
  faq: {
    items: [
      {
        question: "Why does this example not use an Express-specific adapter?",
        answer:
          "auto-geo ships dedicated adapters for Next.js and Hono today, with more on the roadmap. For Express, the cleanest integration is to call runPublish from the package directly inside an express.json route handler. That is the pattern this example documents end to end.",
      },
      {
        question: "Does this work on Express 4 as well as 5?",
        answer:
          "Yes. The example uses Express 5 because it is the current major version with stable async error handling, but runPublish itself is framework-agnostic and works the same on Express 4. The only thing that changes is the Express version pinned in package.json.",
      },
      {
  ...
  disclosure: {
    text: "This is a seed payload included with the auto-geo Express example. It satisfies the hard schema constraints but is deliberately short for demonstration purposes and will surface soft warnings on republish, which mirrors the real iteration loop.",
  },
};
