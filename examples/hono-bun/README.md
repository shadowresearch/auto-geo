# hono-bun — auto-geo on Hono + Bun

Endpoint-only example. Mounts the official `auto-geo/hono` adapter on a
Hono server running under Bun, backed by the in-memory store seeded with
one valid payload.
...
curl http://localhost:3001/api/resources/hello-auto-geo
```
...
```bash
curl -X POST http://localhost:3001/api/resources/publish \
...
```bash
curl http://localhost:3001/api/resources
curl http://localhost:3001/api/resources/publish-test
```
...
| File        | What it does                                                                                |
| ----------- | ------------------------------------------------------------------------------------------- |
| `server.ts` | Hono app: mounts `createHonoRouter` for publish, adds GET endpoints for read-side verifying |
| `seed.ts`   | Inline valid seed payload so the store is non-empty on first run                            |

## Notes

- The `auto-geo/hono` adapter runs the same on Bun, Node, Deno, and
  Cloudflare Workers. Swap the runtime by changing `package.json`
  scripts and the `export default` block in `server.ts`.
